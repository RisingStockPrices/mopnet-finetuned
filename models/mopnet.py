...
# SE BLOCK is for explicit modeling of channel-wise relationships between features
class SEBlock(nn.Module):
    def __init__(self, input_dim, reduction):
        ...
        self.fc = nn.Sequential(
            # two fc layers around the non linearity(relu)
    ...
    def forward(self, x):
        ...
        # apply the new weights to the feature map
        return x * y
...
class Dense_base_down2(nn.Module):
    ...
    # takes as input x (image), edge maps, and labels corresponding to the input image
    def forward(self, x, e, labels):
        ## 256x256
        f0 = self.conv1(x) # 3xWxH -> 64xWxH
        f0_e = self.conv1_e(e) #same for the edge map
        tmp = torch.cat([f0, f0_e], 1) # two 64x... fm -> single 128
        tmp = self.se_tmp(tmp) # se block (no change in dimension)
        f1 = self.conv2(tmp) # 128xwxh -> 64xwxh
        S = f1.size()
        label_0, label_1, label_2 = labels ## see how these labels are used in final removal
        # print(50*'-')
        # print(label_0)
        val_0 = []
        val_1 = []
        ...
        ##########################################
        # so far, f1 holds features modulated(se) and extracted from the input image and edge map

        x1=self.dense_block1(f1) # 64wh->128wh
        x1=self.trans_block1(x1) # 128-> 64 x 128x128
    
        ###  32x32
        x2=(self.dense_block2(x1)) # 64->128
        x2=self.trans_block2(x2) # 128->64x64x64
        # x2 = self.se1(x2)

        x3=(self.dense_block3(x2)) # 64->128
        x3=self.trans_block3(x3) # 128->64x32x32
        # x3 = self.se2(x3)
        ## Classifier  ##

        x4=(self.dense_block4(x3)) # 64->128
        x4=self.trans_block4(x4) #128->64x64x64

        # x4=x4+x2
        x4=torch.cat([x4,x2],1) # 64_64 => 128x64x64
        # x4 = self.se3(x4)
        x5=(self.dense_block5(x4)) # 128 -> 64+128 = 192x64x64
        x5=self.trans_block5(x5) # 192->64x128x128

        # x5=x5+x1
        x5=torch.cat([x5,x1],1) # 128x128x128
        # x5 = self.se4(x5)
        x6=(self.dense_block6(x5))  # 128->64+128=192x128x128
        x6=(self.trans_block6(x6))  # 192->64x256x256
       
        shape_out = x6.data.size() # 128x256x256
        # print(shape_out)
        ...
        # channel-num: 64, 64, 64, 64, 64, 64, spatial dimension 256x256
        r1 = torch.cat([f1, x11, x21, x31, x41, x51, x61], dim=1)
        r1 = self.se5(r1) #still 448x256x256
      
        feature = self.fusion(r1) #64x256x256

        feature = torch.cat([feature, label_0, label_1, label_2], dim=1)#(64+3)x256x256
        feature = self.se6(feature) #67x256x256
        feature = self.fusion2(feature) #64x256x256
        feature = feature + f0 
        self.cnt +=1
        return self.final_conv(feature) #3x256x256