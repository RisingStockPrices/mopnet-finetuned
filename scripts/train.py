...
# create directories for storing intermediate results and checkpoints'
create_exp_dir(opt.exp)
progress_dir=os.path.join(opt.exp,'progress')
checkpoints_dir=os.path.join(opt.exp,'checkpoints')

subdir_name=['GT_edge','pred_edge','input','output','GT']
...
net_label_color=net_label_color.to(device_classifier)

net_label_geo = net.vgg19ca_2()
net_label_geo.load_state_dict(torch.load(opt.netCgeo))
net_label_geo=net_label_geo.to(device_classifier)
...
for epoch in range(opt.epoch_count, opt.niter):
  ...
  for i, data in enumerate(dataloader, 0):
    ...
    # get patchwise classifications
    with torch.no_grad():
      _, label_color = torch.max(net_label_color(input), 1)
      label_curve, label_thick = net_label_geo(input_edge)
      ...
      labels = [label_curve.to(device), label_color.to(device), label_thick.to(device)]
    ...
    if total_steps % 1000 == 0:
      with torch.no_grad():
        ...
        with torch.no_grad():
          ...
          _, label_color = torch.max(net_label_color(val_input), 1)
          label_curve, label_thick = net_label_geo(val_input_edge)
          _, label_curve = torch.max(label_curve, 1)
          _, label_thick = torch.max(label_thick, 1)
          label_curve = label_curve.float()
          label_color = label_color.float()
          label_thick = label_thick.float()
          val_labels = [label_curve.to(device), label_color.to(device), label_thick.to(device)]
